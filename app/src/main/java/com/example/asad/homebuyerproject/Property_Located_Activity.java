    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // Toast.makeText(this, "Index not "+propertyData.contains("selltype"), Toast.LENGTH_SHORT).show();
        ...
        mNextActivity.setOnClickListener(new View.OnClickListener()
        {
            ...
            {
               /* if (!isValidName(fulladdress.getText().toString())){
                    fulladdress.setError("Field  not be Empty");
                }
                else {*/
                ...
                if(isGPSEnabled) {
                    ...
                    if (locationManager != null) {
                        ...
                        if (gpslocation != null) {
                            ...
                            addresses = geocoder.getFromLocation(gpslocation.getLatitude(), gpslocation.getLongitude(), 1); // Here 1 represent max location result to returned, by documents it recommended 1 to 5

                            String address = addresses.get(0).getAddressLine(2); // If any additional address line present than only, check with max available address lines by getMaxAddressLineIndex()
                            String city = addresses.get(0).getLocality();
                            int as=addresses.get(0).getMaxAddressLineIndex();
                            String state = addresses.get(0).getPremises();
                            String country = addresses.get(0).getCountryName();
                            String postalCode = addresses.get(0).getThoroughfare();
                            String knownName = addresses.get(0).getSubLocality();
                            String a = addresses.get(0).getSubAdminArea();
                            System.out.println("Cityname" + address + "||getPremises" + as + "||getThoroughfare" + postalCode
                                    + "||getSubLocality" + knownName + "||getSubAdminArea" + a);*/
                        }
